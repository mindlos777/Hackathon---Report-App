    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 32 }}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subGreeting}>What's happening in your area?</Text>
        </View>
      ...
      >
        {categories.map((cat) => (
          <CategoryButton
            ...
            label={cat.label}
            onPress={() =>
              navigation.navigate("ReportIssue", { presetCategory: cat.id })
            }
          />
        ))}
      </ScrollView>
        ...
        <MapView style={styles.map} initialRegion={region}>
          {reports.map((r) => (
            <Marker
              ...
              title={categories.find((c) => c.id === r.category)?.label}
