...
// Placeholder for a real AI auto-detect call to your Node backend later:
// e.g. POST /api/classify with the photo -> { category, confidence }
...
export default function ReportIssueScreen({ route, navigation }) {
  ...
  const presetCategory = route.params?.presetCategory || null;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // silently ignore in this mock; a real app would surface an error state
      } finally {
        setLocationLoading(false);
      }
    })();
  ...
  const pickPhoto = async (fromCamera) => {
    ...
    if (!permission.granted) {
      Alert.alert("Permission needed", "CivicFix needs access to continue.");
      return;
    }
    ...
    if (!result.canceled) {
      ...
      setPhotoUri(uri);
      // Simulate AI auto-detect once a photo is attached
      const guess = fakeAiDetectCategory();
      setAiSuggested(guess);
      if (!category) setCategory(guess);
    }
  };

  const handleSubmit = () => {
    if (!category) {
      Alert.alert("Select a category", "Please choose the type of issue.");
      return;
    }
    if (!location) {
      Alert.alert(
    ...
    setSubmitting(true);
    // In production this posts to your Node backend, e.g. POST /api/reports
    setTimeout(() => {
      ...
      setSubmitting(false);
      navigation.replace("Tracking", { reportId: report.id });
    }, 800);
  };
    ...
    <ScrollView style={styles.container} contentContainerStyle={{ paddingBottom: 40 }}>
      ...
      <View style={styles.locationBox}>
        {locationLoading ? (
          ...
          <Text style={styles.locationTextMuted}>Location unavailable</Text>
        )}
      </View>
      ...
      {aiSuggested && (
        <Text style={styles.aiHint}>
          AI suggested: {categories.find((c) => c.id === aiSuggested)?.label} — tap to change
        </Text>
      )}
        ...
        {categories.map((cat) => (
          ...
          >
            <Ionicons
              name={cat.icon}
              size={16}
              color={category === cat.id ? "#fff" : colors.primary}
            />
            <Text
              style={[
...
const styles = StyleSheet.create({
  ...
  photoBoxText: { fontSize: 10, color: colors.textMuted, marginTop: 4 },
  ...
  locationTextMuted: { color: colors.textMuted, fontSize: 13 },
  aiHint: { fontSize: 12, color: colors.primary, marginBottom: 8 },
  ...
  categoryChipText: { fontSize: 12, color: colors.primary, marginLeft: 6, fontWeight: "600" },
